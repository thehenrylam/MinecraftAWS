...
###### Jenkins App Credentials
resource "aws_secretsmanager_secret" "scrt_jenkins_credentials_yml" {
    # Initialize the secret container 
    name = "${local.prefix_scrt}-jenkins_credentials_yml"
    description = "Stores the jenkins_credentials.yml field for use in Jenkins (MinecraftAWS)"
    recovery_window_in_days = 0
...
resource "aws_secretsmanager_secret_version" "scrt_jenkins_credentials_yml_version" {
    # Associate the secret to the secret container
    secret_id = aws_secretsmanager_secret.scrt_jenkins_credentials_yml.id
    # Upload the file as the secret_string
    secret_string = file("${var.cache_path}/jenkins_credentials.yml")
}
...
data "aws_iam_policy_document" "iam_policy_document_jenkins" {
    ...
    statement {
        ...
        resources = [
            ...
            aws_secretsmanager_secret.scrt_jenkins_credentials_yml.arn
        ]
    }
}
